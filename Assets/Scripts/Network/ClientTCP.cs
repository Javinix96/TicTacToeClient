...
public class ClientTCP : INetworking
{
    ...
    public async Task Connect(string url, int port, ISession session, PacketRouter router, PopUpController popUpController)
    {
        ...
        {
            _ = PopUpController.ShowPopup("Error de conexión", "Se ha perdido la conexión con el servidor.");
            Debug.Log(e);
            _uiManager.Pop();
        }
    }
    ...
    public async Task Listen()
    {
        ...
        {
            Disconnect();
            await PopUpController.ShowPopup("Exito", "Te has desconectado del servidor");
            _uiManager.PopUPToStart();
        }
    }
