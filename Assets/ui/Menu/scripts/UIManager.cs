...
public class UIManager : MonoBehaviour
{
    ...
    [Header("Layouts")]
    [SerializeField] private VisualTreeAsset roomLayout;
    ...
    public Action<RoomInfoDTO> OnRoomsRequest;
    public Action<string> OnJoinRoom;
    public Action<int> onPlayerReady;
    public Action<int> onStartGame;

    public Action<string> OnSendPassword;

    void OnEnable() => root = GetComponent<UIDocument>().rootVisualElement.Q("Container");

    void Start()
    {
        //     Push(joinRoom, (root2) =>
        //    {    
        //         new PlayerListController(root, this, 2, playerItem, null, _session);
    ...
    public void Lobby(PlayerDTO dto) => GoToJoinRoom(dto, 2);
    public void Pop()
    {
    ...
    public async void GoToRoomList()
    {
        bool answer = false;
        string value = "";

        PlayerData data = await GameManager.Instance.GetPlayerData();

        if (data == null || data.PlayerName == null)
        {
            while (true)
            {
                (answer, value) = await PopUpController.GetTextFieldPopUP("Como te llamas?");
                if (value.Length <= 25)
                    break;
                _ = await PopUpController.ShowPopup("Error", "Demasiado largo tu nombre");
            }

            if (!answer)
            {
                await PopUpController.ShowPopup("Error", "Debes ingresar nombre");
                _networkManager.Disconnect();
            }

            await GameManager.Instance.SaveName(value);
        }
        else
            value = data.PlayerName;

        Packet pck = new Packet();
        pck.WriteInt((int)PacketTypeSend.SendName);
        pck.WriteString(value);
        pck.WriteLength();

        _session.send(pck);

        Push(roomList, (root) => new RoomListController(root, this, _session, roomLayout));
    }
    public void GoToCreateRoom() => Push(createRoom, (root) => new CreateRoomController(root, this, _session));
    public void GoToJoinRoom(PlayerDTO dto, int times = 1)
    {
        if (!dto.Success) return;

        if (dto.RoomHasPassword) return;

        Push(joinRoom, (root) => new PlayerListController(root, this, times, playerItem, dto, _session));
    }
    public void GoBack() => Pop();
    public void BackTimes(int times)
    {
        for (int t = 0; t < times; t++)
            Pop();
    }
    public void PopUPToStart()
    {
        for (int pop = 0; pop <= screenStack.Count; pop++)
            Pop();
    }
}
